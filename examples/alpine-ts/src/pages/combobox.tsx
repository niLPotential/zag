      ...
      <div
        ...
        x-combobox={`{id: $id('combobox'), collection,
            ...
            onInputChange({inputValue}) {
                const filtered = (comboboxData, inputValue, {keys: ['lable']});
                options = filtered.length > 0 ? filtered : comboboxData;},
            ${Object.keys(state)}}`}
        x-combobox:collection={`{items: ${comboboxData}, itemToValue: (item) => item.code, itemToString: (item) => item.label}`}
      >
        ...
        <Toolbar>
          <Controls state={state} controls={comboboxControls} />
          <StateVisualizer />
        </Toolbar>
      </div>
    </main>
